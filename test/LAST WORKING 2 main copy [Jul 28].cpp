...
float rotationThreshold = 200.0; // Full rotation in degrees
unsigned long lastRotationTime = 0;
bool rotationMode = false;
bool hasTriggeredThisRoll = false;
...
void readMPU() {
  ...
  if (Wire.available() >= 14) {
    ...
    float gyroZ = rawGZ / 131.0; // Do a Barrel Roll!
...
void handleMovementTrigger() {
  ...
  if (liquidMode) {
    ...
    pressCount = 0;
    tempoLocked = false;  // Reset tempo lock
  }
...
void checkRotationGesture(float gyroZ) {
  ...
  if (abs(gyroZ) > 0.5) {
    ...
    Serial.print(" | Cumulative: "); Serial.print(cumulativeRotation);
    Serial.print(" | Tilt: "); Serial.println(tiltAngle);
  }
  
  unsigned long currentTime = millis();
  
  // NO TILT RESTRICTION - barrel rolls work at any angle!
  // Accumulate rotation
  if (abs(gyroZ) > 1) { // Rotating fast enough
    cumulativeRotation += gyroZ * 0.01; // Scale by time delta
    lastRotationTime = currentTime;
    rotationMode = true;
    
    Serial.print("🌀 Rotating! Cumulative: "); Serial.println(cumulativeRotation);
    
    if (abs(cumulativeRotation) >= 180.0) {
      // BARREL ROLL DETECTED! 
      if (!hasTriggeredThisRoll) {
        hasTriggeredThisRoll = true;
        
        // ALWAYS return to liquid mode on barrel roll
        Serial.println("🌀🌀🌀 BARREL ROLL DETECTED! Returning to liquid mode! 🌀🌀🌀");
        stopSequence(); // This sets liquidMode = true
        triggerRotationSparkle();
        
        // Also cycle palette for extra visual feedback
        ...
        Serial.println("🎨 Palette also changed to: " + String(currentPaletteIndex));
  ...
  // Reset when back to starting position
  if (abs(cumulativeRotation) < 45.0 || (currentTime - lastRotationTime > 1000)) {
    if (hasTriggeredThisRoll) {
      Serial.println("🔄 Barrel roll reset - ready for next one");
    }
    hasTriggeredThisRoll = false; // Reset the trigger flag
  }
...
void calculateAndUpdateTempo(unsigned long currentTime) {
  ...
  if (pressCount == 2) {
    ...
    // Even better: weighted average of 3 intervals
    unsigned long interval1 = pressHistory[1] - pressHistory[0];
    unsigned long interval2 = pressHistory[2] - pressHistory[1]; 
    ...
    // Stable tempo: use sliding window of last 3 intervals
  ...
  // DRIFT-FREE TIMING: Set next beat time using additive timing
  autoStrobing = true;
  nextBeatTime = currentTime + tempoInterval;  // Next beat at precise time
  lastTempoTime = currentTime;  // Keep this for timeout calculations
