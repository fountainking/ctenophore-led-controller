...
void checkRotationGesture(float gyroZ) {
  ...
  if (abs(tiltAngle) < 0.3) {
    ...
    if (abs(gyroZ) > 1) { // Rotating fast enough
      ...
      // Track barrel roll phases
// static bool hasTriggeredThisRoll = false; // commented out
...
if (abs(cumulativeRotation) >= 180.0) {
  ...
  Serial.println("🌀 Barrel roll palette change: " + String(currentPaletteIndex));
  triggerRotationSparkle();
  
  // Reset rotation counter immediately
